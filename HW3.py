...
class Calculator:
    ...
    def _getPostfix(self, txt):
        ...
        lst=[]
        #attemp to add numbers with decimal values to list 
        temp_num=''
        ...
        #foundation for placing valid items in list from infix expression
        '''
        for char in txt:
            if char != " ":
                lst.append(char)
        ...
        #attemp to check for infix validity  
        #checks if there are too many operators for expression: there should be more numbers than operators 
        '''
        countopr_num=0
        for i in lst:
            if self._isNumber(i):
        ...
        for i in range(len(lst)):
            if not self._isNumber(lst[i]) or lst[i] not in opr: #Check for symbols not in operators
                return None
            if lst[i]=='-':  #check for negative numbers
                if self._isNumber(lst[i-1]):
                    ...
                    postfixStack.push(lst[i])
            elif self._isNumber(lst[i]) and lst[i+1]=='(': #check for invalid form of multiplication ex: 5(2)
                return None
            elif lst[i] in oprnopar and lst[i+1] in oprno_m: #check if two operators are next to each other because it is invalid 
                return None
            else:
                return None
        '''
        ...
        for token in lst:
            ...
            '''
                ...
                while not postfixStack.isEmpty() and oprprior[token]<=oprprior[postfixStack.peek()]: #produces key error
        ...
        #return outputstr
        pass #unable to complete function 
